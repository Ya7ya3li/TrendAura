  ...
  return (
    <div className="w-full bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-[28px] p-5 shadow-xl text-right dir-rtl select-none animate-scale-up">
      ...
      <div className="relative w-full mb-4">
        <span className="absolute right-4 top-4 text-slate-500 text-sm">📎</span>
        <textarea
          value={prompt}
          ...
          disabled={loading}
          className="w-full bg-slate-950/60 text-slate-200 placeholder-slate-600 pr-11 pl-4 pt-4 pb-10 rounded-2xl border border-slate-800 outline-none focus:border-blue-500 transition-all min-h-[110px] resize-none text-xs font-bold leading-relaxed"
        />
        {/* عداد الحروف المجهري الأسفل */}
        <span className="absolute left-4 bottom-3 text-[10px] font-bold text-slate-600 font-sans">
          {prompt?.length || 0}/500
        </span>
      </div>
