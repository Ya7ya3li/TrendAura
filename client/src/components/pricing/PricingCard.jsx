...
/**
 * TrendAura Premium Cyberpunk Pricing Panel - Dual Theme Edition
 */
export default function PricingCard({ plan, isCurrent, userId, onSubscribe, isLoading }) {
  ...
  let cardStyle = "bg-slate-50 dark:bg-[#111625]/90 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white"
  if (isViral) {
    cardStyle = "bg-white dark:bg-[#111625]/95 border-rose-200 dark:border-rose-500 shadow-[0_0_20px_rgba(244,63,94,0.1)] dark:shadow-[0_0_25px_rgba(244,63,94,0.25)] text-slate-900 dark:text-white relative z-10 scale-[1.02] md:-translate-y-1"
  } else if (isPro) {
    cardStyle = "bg-white dark:bg-[#111625]/95 border-blue-200 dark:border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.1)] dark:shadow-[0_0_25px_rgba(59,130,246,0.25)] text-slate-900 dark:text-white relative z-10"
  } else {
    cardStyle = "bg-slate-50 dark:bg-[#111625]/60 border-slate-200 dark:border-slate-800/80 text-slate-900 dark:text-white opacity-95"
  }
    ...
    <div className={`rounded-[28px] p-6 flex flex-col justify-between border transition-all duration-300 relative select-none text-right dir-rtl ${cardStyle}`}>
      ...
      <div>
        ...
        <div className="flex items-center justify-center gap-2 mb-4 mt-2">
          ...
          {isPro && <span className="w-2.5 h-2.5 rounded-full bg-blue-500 dark:bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.9)] shrink-0" />}
          {isFree && <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 dark:bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)] shrink-0" />}
        ...
        <div className="text-center mb-2">
          <span className="text-4xl font-black font-sans tracking-tight transition-colors block">
            {plan.price}
          </span>
          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mt-1 block transition-colors">
            {plan.currency || 'ريال'} / {plan.period || 'شهر'}
          </span>
        </div>

        <p className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 text-center leading-relaxed mb-5 min-h-[32px] px-2 transition-colors">
          {plan.desc}
        </p>

        <div className="border-t border-slate-200 dark:border-slate-800/50 my-4 transition-colors" />

        <div className="mb-6">
          <ul className="space-y-3 text-[10px] font-bold text-right">
            {plan.features?.map((feat, idx) => (
              <li key={idx} className="flex items-start gap-2.5 leading-normal">
                <svg className={`w-3.5 h-3.5 shrink-0 mt-0.5 ${isViral ? 'text-rose-500' : isPro ? 'text-blue-500 dark:text-blue-400' : 'text-emerald-500 dark:text-emerald-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                <span className="text-slate-700 dark:text-slate-300 font-medium transition-colors">{feat}</span>
              </li>
            ))}
          </ul>
        </div>
