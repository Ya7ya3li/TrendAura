...
/**
 * TrendAura Standalone Feature Comparison Matrix Sheet - Pure SVG Edition
 */
export default function PlanComparison() {
  ...
  return (
    <div className="w-full bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-[28px] p-5 shadow-2xl text-right dir-rtl select-none font-sans mt-12 animate-fade-in">
      ...
      <div className="flex items-center gap-2 mb-6 pb-3 border-b border-slate-800/60">
        ...
        <h3 className="text-xs font-black text-white tracking-tight">جدول المقارنة الفنية والامتيازات الشاملة</h3>
      </div>
        ...
        <table className="w-full text-right text-[11px] font-bold text-slate-400 whitespace-nowrap">
          <thead>
            <tr className="border-b border-slate-800 text-slate-500 text-[10px] uppercase">
              ...
              <th className="pb-3">الخطة الحرة</th>
          ...
          <tbody className="divide-y divide-slate-900/40">
            {comparisonSpecs.map((spec, idx) => (
              <tr key={idx} className="hover:bg-slate-900/10 transition-colors">
                <td className="py-3.5 font-black text-white text-right">{spec.name}</td>
