...
export default function MobileSidebar() {
  ...
  return (
    <div className="md:hidden fixed inset-0 z-50 flex dir-rtl text-right select-none">
      ...
      <div className="relative w-64 max-w-sm h-full bg-white dark:bg-slate-950 border-l border-slate-200 dark:border-slate-900 shadow-2xl p-5 flex flex-col justify-between z-10 animate-slide-right mr-0 transition-colors duration-300">
        
        <div>
          {/* هيدر اللوجو مع زر الإغلاق */}
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-900 pb-4 mb-5">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-xs font-black text-slate-900 dark:text-white">TrendAura Mobile</span>
            </div>
            
            <button 
              onClick={() => setSidebarOpen(false)} 
              className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 flex items-center justify-center active:scale-95 transition-all"
            >
