...
export default function SidebarItem({ to, label, icon, external, onClick }) {
  ...
  if (external) {
    return (
      <a 
        ...
        className={`${baseClass} text-slate-400 hover:bg-slate-800/40 hover:text-white border border-transparent`}
        onClick={onClick}
      >
        <span className="w-4 h-4 text-slate-400 shrink-0">{icon}</span>
  ...
  // 🔄 معالجة الروابط الداخلية الصاروخية داخل شبكة بيئة React الافتراضية
  return (
    <NavLink
      to={to}
      onClick={onClick}
      className={({ isActive }) => 
        isActive 
          ...
          : `${baseClass} text-slate-400 hover:bg-slate-800/40 hover:text-slate-200`
      }
    >
      {({ isActive }) => (
        <>
          <span className={`w-4 h-4 shrink-0 transition-transform ${isActive ? 'scale-110 text-white' : 'text-slate-400'}`}>
            {icon}
          </span>
