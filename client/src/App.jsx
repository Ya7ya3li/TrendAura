        ...
        <SubscriptionProvider>
          <ThemeProvider>
            <AppProvider>
              ...
              {toast.show && (
                <div className={`fixed bottom-6 left-6 z-[9999] px-6 py-3 rounded-2xl shadow-2xl text-xs font-bold text-white backdrop-blur-md border animate-fade-in flex items-center gap-2 ${toast.type === 'success' ? 'bg-emerald-600/90 border-emerald-500/30' : 'bg-rose-600/90 border-rose-500/30'
                  }`}>
...
function AppContent() {
  ...
  if (!initialized) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center dir-rtl transition-colors duration-300">
        <div className="flex flex-col items-center gap-4">
          <svg className="w-10 h-10 border-4 border-blue-600 dark:border-blue-500 border-t-transparent rounded-full animate-spin" />
          <p className="text-slate-500 dark:text-slate-400 text-xs font-bold tracking-wide animate-pulse">جاري تهيئة الخوادم ...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans antialiased relative selection:bg-blue-500/20 selection:text-blue-500 dark:selection:text-blue-400 transition-colors duration-300">
      ...
      <Routes>
        ...
        {/* 👑 مسار الإدارة المركزية (Root Owner) محمي فولاذياً */}
