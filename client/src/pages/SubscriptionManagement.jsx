...
export default function SubscriptionManagement() {
  ...
  const triggerMoyasarCheckout = async (amount, planId, tokensCount) => {
    ...
    try {
      // 🚀 استدعاء وتأمين الحقن من السيرفر الحي المربوط بريلواي لتهيئة جلسة دفع ميسر الملوكية
      const res = await paymentService.createInvoice(amount, planId, profile.id, tokensCount)
      if (res && res.invoiceUrl) {
        window.location.href = res.invoiceUrl // تحويل آمن فوري لصفحة البنك المشفرة
      } else {
        throw new Error("رابط الدفع مفقود من الخادم")
      }
    } catch (err) {
      if (typeof showToast === 'function') {
        showToast('فشل تأمين وحقن رابط دفع ميسر يرجى اعادة المحاولة لاحقاً', 'error')
      }
    } finally {
